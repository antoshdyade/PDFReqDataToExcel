...
def extract_data_from_pdf(pdf_path):
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for page_num, page in enumerate(pdf.pages):
            ...
            if len(text) > 1 and "COEP" in text[1]:  # Check if the second line contains "COEP"
                for line in text:
                    if 'PRN:' in line:
                    ...
                    if prn and name:
                        ...
                        for subsequent_line in text[text.index(line)+1:]:
                            if 'BTN03405' in subsequent_line:
                                ...
                                if btn03405_count == 1:
                                    try:
                                        ise_obt = subject_data[4].strip()  # ISE(Obt)
                                    except IndexError:
                                        ise_obt = None
                                elif btn03405_count == 2:
                                    try:
                                        ica_obt = subject_data[4].strip()  # ICA(Obt)
                                        poe_obt = subject_data[6].strip()  # POE(Obt)
                                    except IndexError:
                                        ica_obt = None
                                        poe_obt = None
                                elif btn03405_count == 3:
                                    try:
                                        total_obt = subject_data[4].strip()  # Total(Obt)
                                    except IndexError:
                                        total_obt = None
                                    break
...
pdf_path = 'C:/Users/Antosh/Downloads/r8/result.pdf'  # Replace with your PDF file path
excel_path = 'C:/Users/Antosh/Downloads/r8/extracted_data.xlsx'  # Replace with your desired output Excel file path
